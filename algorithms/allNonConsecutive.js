...
function allNonConsecutive(sortedNums) {
    ...
    for (let i = 1; i < sortedNums.length; i++) {
        // if current element is not 1 more than previous, it's non-consecutive
        if (sortedNums[i] !== sortedNums[i - 1] + 1)
            // push non-consecutive index and element as object to result array 
            res.push({ i, n: sortedNums[i] });
    }
    // return result array
    return res;
} 
